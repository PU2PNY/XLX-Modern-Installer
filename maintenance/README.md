...
- The executor accepts only the fixed allowlist `readonly_smoke`, `backup_only`, `golden_lab`, `candidate_build`, and `candidate_finalize`.
...
- `candidate_finalize` only validates and finalizes an already-built candidate whose first run reached the known checksum-name typo after successful build/static audit. It does not compile again, execute the candidate, install it, or restart production.
- Root operations call only fixed, root-owned local wrappers installed from a commit-pinned bootstrap/upgrade with SHA-256 validation.
