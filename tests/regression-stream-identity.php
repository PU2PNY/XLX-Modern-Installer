...
$r=xlxmodern_exact_tx_origin($wrongModule,'PU2OJI','B','C');
expect_same($r['origin_match'],'log-client','different module is rejected');
...
$preserved=xlxmodern_enforce_safe_origin($station,$other);
expect_same($preserved,$station,'STATION identity is preserved');

$source=file_get_contents(dirname(__DIR__).'/dashboard/assets/ao-vivo-authorized-sync-v1.js');
foreach(['Nº','País','Status','Indicativo','Nome','Hotspot / Repetidora','Cidade','Protocolo','Módulo','Tempo de transmissão'] as $label){
    if(strpos((string)$source,$label)===false){ fwrite(STDERR,"FAIL: missing history column $label\n"); exit(1); }
}
if(strpos((string)$source,"source.indexOf('xlxd-station') === 0")===false){ fwrite(STDERR,"FAIL: Link must require STATION source\n"); exit(1); }

$live=file_get_contents(dirname(__DIR__).'/dashboard/api/live.php');
if(strpos((string)$live,"'gateway'=>$call")===false){ fwrite(STDERR,"FAIL: live gateway must start from XLXD client\n"); exit(1); }
if(strpos((string)$live,"$connection['via']\n                ?? $connection['peer']")!==false){ fwrite(STDERR,"FAIL: live must not promote via/peer to gateway\n"); exit(1); }
