#!/usr/bin/env python3
import argparse, datetime as dt, json, os, re, socket, struct, tempfile, time
IFACE='ens192'; PORT=62030
STATE='/var/lib/xlx026-dmr-meta/metadata.json'
...
def udp_payload(frame):
    ...
    return frame[u+8:u+ln],socket.inet_ntoa(frame[off+12:off+16])
def load_state():
    try:
...
def selftest():
    rid=7241234; cfg=(f"{'PU2PNY':<8}{'438800000':>9}{'431200000':>9}{'10':>2}{'01':>2}{'-23.3121':>8}{'-46.2248':>9}{'750':>3}{'Santa Isabel, SP':<20.20}{'XLX026 test':<19.19}{'4'}{'https://xlx026.net':<124.124}{'20260907':<40.40}{'MMDVM':<40.40}").encode('ascii')
    p=b'RPTC'+rid.to_bytes(4,'big')+cfg
    assert len(p)==302,len(p)
    x=parse_rptc(p); assert x and x['callsign']=='PU2PNY' and x['dmr_id']==rid and x['color_code']==1 and x['location']=='Santa Isabel, SP',x
