<?php /* {{REFLECTOR_NAME}} — Simulado nativo; shell vem integralmente do painel. */ ?>
...
<section class="simulado-hero" aria-labelledby="simuladoTitle">
    ...
    <div class="not-official" role="note" aria-label="Aviso de simulação educacional">
      ...
      <p><strong>Transparência:</strong> este simulador é um material independente do {{REFLECTOR_NAME}} Brasil para estudo. As questões são de treinamento, elaboradas a partir do conteúdo e de normas oficiais vigentes. <strong>Elas não são apresentadas como perguntas reais, copiadas ou integrantes do banco interno utilizado pela ANATEL nas avaliações.</strong></p>
    </div>
  </section>
  ...
  <section class="sources" aria-labelledby="sourcesTitle">
    ...
    <p>As referências aparecem também na correção das questões. O {{REFLECTOR_NAME}} usa fontes normativas para sustentar as respostas, mas isso <strong>não transforma as questões de treinamento em questões oficiais da prova</strong>.</p>
    <ul>
  ...
  <section class="study-guide" aria-labelledby="guideTitle">
    ...
    <div class="faq" aria-label="Perguntas frequentes sobre o simulado">
      ...
      <details><summary>O simulador é da ANATEL?</summary><p>Não. É um projeto educacional independente do {{REFLECTOR_NAME}} Brasil. As referências externas levam às páginas oficiais para permitir que o candidato confirme a regulamentação utilizada.</p></details>
