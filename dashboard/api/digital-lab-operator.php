...
session_name('XLXMODERN_DLAB_OPERATOR');
...
if ($method === 'GET') {
    ...
    if ($action === 'status') {
        ...
        if ($perm['send_aprs']) {
            ...
            /*
             * Contas comuns podem transmitir, mas nao recebem o
             * historico global do indicativo de servico.
             */
            if (
                is_array($operator) &&
                strtoupper((string)$user['role']) !== 'ADMIN'
            ) {
                unset($operator['messages']);
            }
        }
