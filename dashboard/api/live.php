...
$internalMtr = (($_GET['mtr'] ?? '') === '1') && in_array($remote, ['127.0.0.1','::1'], true);
...
if (is_readable($statusCache)) {
    ...
    if (is_array($cached) && isset($cached['connections']) && is_array($cached['connections'])) {
...
function live_timestamp(string $line): int {
    if (preg_match('/^(\d{1,2})\s+([A-Za-z]{3}),\s+(\d{2}):(\d{2}):(\d{2}):/', $line, $m)) {
        ...
        if ($month) {
            $year=(int)date('Y');
            $ts=mktime((int)$m[3],(int)$m[4],(int)$m[5],$month,(int)$m[1],$year);
            if ($ts>time()+86400) $ts=mktime((int)$m[3],(int)$m[4],(int)$m[5],$month,(int)$m[1],$year-1);
            return $ts;
        }
    }
    return time();
...
function live_match(array $connections,string $call,string $suffix,string $module,int $ts): ?array {
    ...
    foreach($connections as $c){
        $s=0;
        if(($c['callsign']??'')===$call)$s+=100;
        if(($c['module']??'')===$module)$s+=70;
        if($suffix!==''&&($c['suffix']??'')===$suffix)$s+=20;
        $age=abs($ts-(int)($c['last_activity']??0));
        if($age<=10)$s+=30;elseif($age<=120)$s+=10;
...
if($handle===false){http_response_code(503);echo json_encode(['ok'=>false,'error'=>'log_open_failed']);exit;}
$size=filesize($logFile)?:0;$read=min($size,131072);$start=max(0,$size-$read);
if($start>0){fseek($handle,$start);fgets($handle);} $raw=stream_get_contents($handle)?:'';fclose($handle);

$active=[];$recent=[];
foreach(preg_split('/\R/',$raw)?:[] as $line){
    if(preg_match('/New client\s+([A-Z0-9]+)(?:\s+([A-Z0-9]+))?.*?protocol\s+([A-Za-z0-9+_-]+)(?:.*?module\s+([A-Z]))?/i',$line,$m)){
        $call=strtoupper(trim($m[1]));$suffix=strtoupper(trim($m[2]??''));$module=strtoupper(trim($m[4]??'?'));
        $recent[$call.'|'.$suffix.'|'.$module]=strtoupper(trim($m[3]));
        $recent[$call.'||'.$module]=strtoupper(trim($m[3]));
    }
    if(preg_match('/Opening stream on module\s+([A-Z])\s+for client\s+([A-Z0-9]+)\s*([A-Z0-9]+)?\s+with sid\s+(\d+)/i',$line,$m)){
        $module=strtoupper($m[1]);$call=strtoupper(trim($m[2]));$suffix=strtoupper(trim($m[3]??''));$sid=(int)$m[4];$ts=live_timestamp($line);
        $c=live_match($connections,$call,$suffix,$module,$ts);
        $protocol=(string)($c['protocol']??($recent[$call.'|'.$suffix.'|'.$module]??$recent[$call.'||'.$module]??($config['modules'][$module]['protocol']??'Não identificado')));
        $gateway=trim((string)($c['via']??''));
        if($gateway==='')$gateway=trim((string)($c['peer']??''));
        if($gateway==='')$gateway=$call;
        $active[$module]=[
            'key'=>$module.':'.$sid,'module'=>$module,'stream_id'=>$sid,'callsign'=>$call,'suffix'=>$suffix,
            'name'=>(string)($c['name']??$call),'location'=>(string)($c['location']??'Não informada'),
            'country'=>(array)($c['country']??live_country($call)),'protocol'=>$protocol,'started_at'=>$ts,
            'qrz'=>'https://www.qrz.com/db/'.rawurlencode($call),'gateway'=>$gateway,
            'endpoint_ip'=>'','state'=>'transmitting','ip'=>$internalMtr?(string)($c['ip']??''):''
        ];
    }
