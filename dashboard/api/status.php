...
require __DIR__ . '/identity-safe.php';
...
$requestedHistoryLimit = isset($_GET['history']) ? (int)$_GET['history'] : $defaultHistoryLimit;
$requestedHistoryHours = isset($_GET['history_hours']) ? (int)$_GET['history_hours'] : 0;
$is24HourHistory = $requestedHistoryHours === 24;
$historyLimit = $is24HourHistory ? 5000 : (in_array($requestedHistoryLimit,[30,150],true) ? $requestedHistoryLimit : $defaultHistoryLimit);
$historySince = $is24HourHistory ? time()-86400 : null;
$cacheVariant = $is24HourHistory ? '-24h' : ($historyLimit===$defaultHistoryLimit ? '' : '-'.$historyLimit);
...
function send_cached_status(string $cacheFile,int $cacheTtl,bool $allowExpired=false): bool {
    ...
    if(!is_array($decoded) || empty($decoded['ok'])) return false;
    header('X-{{REFLECTOR_NAME}}-Cache: '.($allowExpired?'stale-while-refresh':'fresh'));
    echo $json;
    return true;
}
...
if($lockHandle===false){
    ...
    echo json_encode(['ok'=>false,'error'=>'status_lock_unavailable'],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
    exit;
}
...
try {
    if(!flock($lockHandle,LOCK_EX|LOCK_NB)){
        ...
        fclose($lockHandle);
        http_response_code(503);
        echo json_encode(['ok'=>false,'error'=>'status_refresh_in_progress'],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
        exit;
    }

    if(send_cached_status($cacheFile,$cacheTtl)){ flock($lockHandle,LOCK_UN); fclose($lockHandle); exit; }

    $connections=array_map('xlx_user_directory_apply',parse_xml_connections_sync());

    /*
     * common.php contains the current STATION stream identity implementation,
     * including strict non-D-STAR matching and ambiguity rejection.
     */
    $tx=active_and_history($connections,$historyLimit,$historySince);

    foreach($tx['active'] as $module=>$transmission){
        $tx['active'][$module]=xlx_user_directory_apply(
            xlxmodern_enforce_safe_origin($transmission,$connections)
        );
    }
    $tx['history']=array_map(
        static fn(array $item): array => xlx_user_directory_apply(xlxmodern_enforce_safe_origin($item,$connections)),
        $tx['history']
    );
    ...
    foreach(cfg()['modules'] as $letter=>$meta){
        ...
        foreach($tx['history'] as $historyItem){
            if($historyItem['module']===$letter){ $lastTransmission=$historyItem; break; }
        }
        $modules[$letter]=[
            'module'=>$letter,
            'name'=>$meta['name'],
    ...
    $history=array_map(
        static function(array $historyItem) use($online): array {
            ...
            $historyItem['status']=xlxmodern_history_state($historyItem);
            return $historyItem;
        },
        $tx['history']
    );
    ...
    if($json===false) throw new RuntimeException('Failed to generate status JSON.');

    $temporaryFile=$cacheFile.'.'.getmypid().'.tmp';
    if(file_put_contents($temporaryFile,$json,LOCK_EX)===false) throw new RuntimeException('Failed to write temporary status cache.');
    if(!rename($temporaryFile,$cacheFile)){ @unlink($temporaryFile); throw new RuntimeException('Failed to publish status cache.'); }
    ...
    if(is_readable($cacheFile)){
        ...
        if($fallback!==false && $fallback!==''){ echo $fallback; exit; }
    }
    http_response_code(503);
