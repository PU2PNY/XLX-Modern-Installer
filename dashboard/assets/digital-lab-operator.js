...
const api='/api/digital-lab-operator.php';
...
function prepareAprsReply(rawCall){
  ...
  if(
    ...
    notify(
      'Entre com sua conta para responder pelo painel.'
    );
    return;
  }
...
function applyPermissions(){
  ...
  form.hidden=!canSend;
  historyWrap.hidden=!canViewPrivate;
  soundToggle.hidden=!canViewPrivate;
  unreadBadge.hidden=
    !canViewPrivate || unread<1;
...
async function refresh(){
  try{
    ...
    if(permissions.send_aprs){
      ...
      const roleName=
        String(account?.role||'USER').toUpperCase();
      ...
      state.textContent=connected
        ? (
            tx
              ? `${roleLabel} • APRS-IS verificado • TX habilitado`
              : `${roleLabel} • APRS-IS verificado • TX bloqueado`
          )
        : `${roleLabel} • APRS-IS não está pronto`;
...
var dlabFaqButtons=root.querySelectorAll('.dlab-faq-toggle');

function dlabFaqClose(button){
  var panelId=button.getAttribute('aria-controls');
  var panel=panelId
    ? document.getElementById(panelId)
    : null;
  var icon=button.querySelector('.dlab-faq-icon');
...
function dlabFaqOpen(button){
  ...
  for(var i=0;i<dlabFaqButtons.length;i++){
    if(dlabFaqButtons[i]!==button){
      dlabFaqClose(dlabFaqButtons[i]);
    }
  }
...
for(var dlabFaqIndex=0;
    dlabFaqIndex<dlabFaqButtons.length;
    dlabFaqIndex++){
