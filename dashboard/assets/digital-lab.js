/* {{REFLECTOR_NAME}} DIGITAL LAB V1 — JS isolado; não altera app.js */
(()=>{
'use strict';
...
const focusStation=upperFromQuery();let focusOpened=false;
function upperFromQuery(){try{return new URLSearchParams(location.search).get('station')?.toUpperCase().replace(/[^A-Z0-9-]/g,'').slice(0,11)||''}catch(_e){return ''}}
...
function sourceLabel(s){if(String(s||'').startsWith('DPRS_MODULE_'))return 'D-PRS • '+String(s).slice(-1);return s==='APRS_IS'?'APRS-IS':s==='YSF_GPS'?'YSF GPS':s==='DMR_GPS'?'DMR GPS':txt(s)||'DATA'}
...
function startTest(call){test={call,start:Date.now(),expires:Date.now()+120000};els.cancel.hidden=false;setTest('',`Aguardando ${call}…`,'Transmita pelo {{REFLECTOR_NAME}}-B com GPS/D-PRS ativado. O teste fica ativo por 2 minutos.');schedule(250)}
function stopTest(message='Teste cancelado.'){test=null;els.cancel.hidden=true;setTest('','Pronto para testar',message)}
function evaluateTest(data){if(!test)return;const stations=Array.isArray(data?.stations)?data.stations:[];const found=stations.find(s=>upperCall(s.callsign)===test.call&&s.source==='DPRS_MODULE_B'&&ts(s.last_seen)>=test.start-1500);if(found){setTest('success',`Beacon de ${test.call} recebido!`,`D-PRS confirmado no módulo B • ${coord(found.lat)}, ${coord(found.lon)} • ${age(found.last_seen)}`);els.cancel.hidden=true;test=null;showToast(`✅ ${found.callsign}: beacon D-PRS recebido pelo {{REFLECTOR_NAME}}-B.`);return}if(Date.now()>test.expires){setTest('error','Beacon não encontrado','O período de 2 minutos terminou. Confira se o rádio está no módulo B e se GPS/D-PRS está ativado.');els.cancel.hidden=true;test=null}}
async function load(){try{const res=await fetch(api,{cache:'no-store',headers:{Accept:'application/json'}});const data=await res.json();latest=data;if(!res.ok||data.ok===false){renderOffline(data);return}renderStatus(data);renderStations(data.stations);renderCommands(data.commands);if(focusStation&&!focusOpened){const st=(Array.isArray(data.stations)?data.stations:[]).find(x=>upperCall(x.callsign)===focusStation);if(st){focusOpened=true;openMap(st);document.querySelector('.dlab-map-card')?.scrollIntoView({behavior:'smooth',block:'center'})}}evaluateTest(data)}catch(err){renderOffline({message:'Não foi possível ler o Digital Lab.'})}finally{schedule()}}
