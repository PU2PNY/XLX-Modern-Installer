/* XLXMODERN DIGITAL LAB V1 — JS isolado; não altera app.js */
(()=>{
'use strict';
...
function startTest(call){test={call,start:Date.now(),expires:Date.now()+120000};els.cancel.hidden=false;setTest('',`Aguardando ${call}…`,'Transmita pelo XLXMODERN-B com GPS/D-PRS ativado. O teste fica ativo por 2 minutos.');schedule(250)}
function stopTest(message='Teste cancelado.'){test=null;els.cancel.hidden=true;setTest('','Pronto para testar',message)}
function evaluateTest(data){if(!test)return;const stations=Array.isArray(data?.stations)?data.stations:[];const found=stations.find(s=>upperCall(s.callsign)===test.call&&s.source==='DPRS_MODULE_B'&&ts(s.last_seen)>=test.start-1500);if(found){setTest('success',`Beacon de ${test.call} recebido!`,`D-PRS confirmado no módulo B • ${coord(found.lat)}, ${coord(found.lon)} • ${age(found.last_seen)}`);els.cancel.hidden=true;test=null;showToast(`✅ ${found.callsign}: beacon D-PRS recebido pelo XLXMODERN-B.`);return}if(Date.now()>test.expires){setTest('error','Beacon não encontrado','O período de 2 minutos terminou. Confira se o rádio está no módulo B e se GPS/D-PRS está ativado.');els.cancel.hidden=true;test=null}}
