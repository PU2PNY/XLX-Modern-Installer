...
for c in awk chown chmod cp curl cut date diff df find git grep head install journalctl mv pgrep python3 readlink rm sed sha256sum sleep sort ss stat systemctl tail tar tee wc xargs; do
...
xml_version(){
  sed -n 's:.*<Version>\([^<]*\)</Version>.*:\1:p' /var/log/xlxd.xml 2>/dev/null | head -1
}

capture_xlxd_listeners(){
  local out="$1"
  ss -H -lunp 2>/dev/null | grep 'xlxd' | awk '{print $5}' | sort -u > "$out" || true
}
...
http_matrix(){
  ...
  mkdir -p "$dir"
  local pages=(
    "/ao-vivo" "/conectados" "/suporte" "/aprs-dprs" "/ranking"
    "/certificado" "/simulado-anatel/" "/refletores" "/noticias"
  )
...
rollback(){
  ...
  if [[ "$SOURCE_MUTATED" -eq 1 && -n "$OLD_SRC_RENAMED" && -d "$OLD_SRC_RENAMED" ]]; then
    rm -rf "$PROD_SRC.rollback-new" 2>/dev/null
    if [[ -e "$PROD_SRC" ]]; then
      mv "$PROD_SRC" "$PROD_SRC.rollback-new" 2>/dev/null || true
    fi
    mv "$OLD_SRC_RENAMED" "$PROD_SRC" 2>/dev/null || true
  fi

  if [[ "$MUTATED" -eq 1 && -f "$OUT/production-xlxd.before" ]]; then
    cp -a "$OUT/production-xlxd.before" "$OUT/.xlxd.rollback"
    chown --reference="$OUT/production-xlxd.before" "$OUT/.xlxd.rollback" 2>/dev/null || true
    chmod --reference="$OUT/production-xlxd.before" "$OUT/.xlxd.rollback" 2>/dev/null || true
    mv -f "$OUT/.xlxd.rollback" "$PROD_BIN"
    systemctl restart "$SERVICE" || true
    for _ in {1..30}; do
      systemctl is-active --quiet "$SERVICE" && [[ "$(sha256sum "$PROD_BIN" 2>/dev/null | awk '{print $1}')" == "$EXPECTED_OLD_SHA" ]] && [[ "$(xml_version)" == "$EXPECTED_OLD_VERSION" ]] && break
      sleep 1
    done
  fi
...
xlxd_service=ACTIVE
EOF
    echo "[OK] Rollback confirmado: XLXD 2.5.3 restaurado e ativo."
  else
...
production_xml_version=$CURRENT_VER
EOF
    echo "[ERRO] Rollback não pôde ser confirmado automaticamente." >&2
  fi
  exit 90
}

trap 'rc=$?; if [[ $rc -ne 0 && $SUCCESS -ne 1 ]]; then rollback "falha inesperada rc=$rc"; fi' EXIT

section "1/11 — FINALIZAR CANDIDATO SE NECESSÁRIO"
...
section "2/11 — EXECUTAR PRÉ-PUBLICAÇÃO FAIL-CLOSED"
"$PRECHECK"
...
section "3/11 — BACKUP IMEDIATO E MANIFESTO"
AVAIL_KB="$(df -Pk /root | awk 'NR==2 {print $4}')"
[[ "${AVAIL_KB:-0}" -ge 1048576 ]] || { fail "menos de 1 GiB livre antes do backup"; exit 30; }

cp -a "$PROD_BIN" "$OUT/production-xlxd.before"
sha256sum "$OUT/production-xlxd.before" > "$OUT/production-xlxd.before.sha256"
(
  cd "$OUT"
  sha256sum -c production-xlxd.before.sha256
)
...
tail -n 400 /var/log/xlx.log > "$OUT/xlx.log.tail.before.txt" 2>/dev/null || true
capture_xlxd_listeners "$OUT/listeners.before.txt"
[[ -s "$OUT/listeners.before.txt" ]] || { fail "não foi possível capturar listeners baseline"; exit 31; }
http_matrix "baseline imediato" "$OUT/http/before"
ok "Backup imediato verificado"

section "4/11 — PREPARAR BINÁRIO ATÔMICO"
OWNER_UID="$(stat -c '%u' "$PROD_BIN")"
OWNER_GID="$(stat -c '%g' "$PROD_BIN")"
MODE="$(stat -c '%a' "$PROD_BIN")"
TMP_BIN="/xlxd/.xlxd.release-${STAMP}"
install -o "$OWNER_UID" -g "$OWNER_GID" -m "$MODE" "$CAND_BIN" "$TMP_BIN"
[[ "$(sha256sum "$TMP_BIN" | awk '{print $1}')" == "$CAND_SHA" ]] || { fail "SHA do binário temporário diverge"; exit 40; }
ok "Binário candidato preparado no mesmo filesystem"

section "5/11 — PUBLICAÇÃO ATÔMICA E RESTART SOMENTE DO XLXD"
RESTART_AT="$(date --iso-8601=seconds)"
MUTATED=1
mv -f "$TMP_BIN" "$PROD_BIN"
TMP_BIN=""
[[ "$(sha256sum "$PROD_BIN" | awk '{print $1}')" == "$CAND_SHA" ]] || { fail "SHA após troca atômica diverge"; exit 50; }
systemctl restart "$SERVICE"

for _ in {1..35}; do
  if systemctl is-active --quiet "$SERVICE" && [[ "$(pgrep -x xlxd | wc -l)" -eq 1 ]]; then
    break
  fi
  sleep 1
done
systemctl is-active --quiet "$SERVICE" || { fail "xlxd.service não voltou active"; exit 51; }
[[ "$(pgrep -x xlxd | wc -l)" -eq 1 ]] || { fail "quantidade de processos xlxd pós-restart inválida"; exit 52; }
[[ "$(sha256sum "$PROD_BIN" | awk '{print $1}')" == "$CAND_SHA" ]] || { fail "binário ativo não é o candidato"; exit 53; }
ok "XLXD reiniciado com o candidato"

section "6/11 — XML 2.6.0 E LISTENERS"
NEW_XML=""
for _ in {1..45}; do
  NEW_XML="$(xml_version)"
  [[ "$NEW_XML" == "$EXPECTED_NEW_VERSION" ]] && break
  sleep 1
done
[[ "$NEW_XML" == "$EXPECTED_NEW_VERSION" ]] || { fail "XML não passou para 2.6.0; atual=$NEW_XML"; exit 60; }
cp -a /var/log/xlxd.xml "$OUT/xlxd.xml.after"

for _ in {1..30}; do
  capture_xlxd_listeners "$OUT/listeners.after.txt"
  if diff -u "$OUT/listeners.before.txt" "$OUT/listeners.after.txt" > "$OUT/listeners.diff.txt"; then
    break
  fi
  sleep 1
done
if ! diff -u "$OUT/listeners.before.txt" "$OUT/listeners.after.txt" > "$OUT/listeners.diff.txt"; then
  cat "$OUT/listeners.diff.txt"
  fail "conjunto de listeners UDP do XLXD mudou"
  exit 61
fi
if grep -Eq '(^|:)17000$' "$OUT/listeners.after.txt"; then
  fail "porta M17 17000 apareceu após publicação"
  exit 62
fi
ok "XML 2.6.0 e conjunto de listeners preservados"

section "7/11 — LOGS PÓS-RESTART"
journalctl -u "$SERVICE" --since "$RESTART_AT" --no-pager > "$OUT/xlxd.journal.after.txt" 2>&1 || true
tail -n 500 /var/log/xlx.log > "$OUT/xlx.log.tail.after.txt" 2>/dev/null || true
if grep -Eqi 'Address already in use|bind[^[:alnum:]]+failed|segmentation fault|core dumped|terminate called|fatal error' "$OUT/xlxd.journal.after.txt" "$OUT/xlx.log.tail.after.txt"; then
  fail "erro crítico detectado nos logs pós-restart"
  exit 70
fi
ok "Sem erro crítico pós-restart nos padrões bloqueantes"

section "8/11 — REGRESSÃO HTTP/APIS"
http_matrix "pós-publicação" "$OUT/http/after" || { fail "regressão HTTP/API detectada"; exit 80; }
ok "Painel e APIs continuam operacionais"

section "9/11 — SINCRONIZAR FONTE /usr/src/xlxd"
...
mv "$TMP_SRC" "$PROD_SRC"
TMP_SRC=""
[[ "$(git -C "$PROD_SRC" rev-parse HEAD)" == "$EXPECTED_UPSTREAM" ]] || { fail "fonte publicada com HEAD inesperado"; exit 91; }
...
ok "Fonte de produção sincronizada com o candidato customizado"

section "10/11 — VALIDAÇÃO FINAL DE PRODUÇÃO"
systemctl is-active --quiet "$SERVICE" || { fail "serviço deixou de estar ativo"; exit 100; }
[[ "$(sha256sum "$PROD_BIN" | awk '{print $1}')" == "$CAND_SHA" ]] || { fail "SHA final do binário diverge"; exit 101; }
...
http_matrix "validação final" "$OUT/http/final" || { fail "painel/APIs falharam na validação final"; exit 104; }
ok "Core 2.6.0 validado em produção"

section "11/11 — MARCADOR, HASHES E ROLLBACK PRESERVADO"
find "$OUT" -type f \
  ! -name 'execution.log' \
  ! -name 'RELEASE_COMPLETE' \
  ! -name 'RELEASE-MANIFEST.sha256' \
  -print0 | sort -z | xargs -0 sha256sum > "$OUT/RELEASE-MANIFEST.sha256"
