...
access_post = r''' if(in_array($a,['access-add-white','access-delete-white','access-add-black','access-delete-black'],true)){
 ...
 if($a==='access-save-terminal'){
  $p=(string)($_POST['access_password']??'');$c=($_POST['confirm_access']??'')==='yes';$address=posted('terminal_address',253);$modules=strtoupper(preg_replace('/[^A-Za-z*]/','',(string)($_POST['terminal_modules']??''))??'');
  if(!$c||!password_verify($p,$cfg['password_hash'])){$msg='Alteração cancelada: confirmação ou senha inválida.';$bad=true;audit('terminal_denied');}
  else{[$aok,$aj,$detail]=jr('access-save-terminal',[$address,$modules]);if($aok){$access=$aj;$accessOk=true;$msg='Terminal XLXD salvo. Reinicie o XLXD somente se quiser aplicar imediatamente.';audit('terminal_save');}else{$msg='Não foi possível salvar: '.mb_substr($detail,0,140);$bad=true;audit('terminal_fail');}}
 }
'''
...
quick = r'''<section class="p" id="quick-links"><div class="section-head"><div><h2>Links rápidos</h2><p class="sub">Atalhos internos para as páginas públicas do refletor.</p></div><span class="badge">Somente após login</span></div><div class="actions"><a class="btn" href="/ao-vivo">Ao Vivo</a><a class="btn secondary" href="/conectados">Conectados</a><a class="btn secondary" href="/ranking">Ranking</a><a class="btn secondary" href="/refletores">Refletores</a><?php if(is_dir(dirname(__DIR__).'/aprs-dprs')):?><a class="btn secondary" href="/aprs-dprs/">APRS / D-PRS</a><?php endif;?></div></section>
'''
once('<section class="p"><h2>Integridade e testes</h2>', quick + '<section class="p"><h2>Integridade e testes</h2>')

access_ui = r'''<section class="p" id="access"><div class="section-head"><div><h2>Controle de Acesso XLXD</h2><p class="sub">Whitelist, blacklist e rota terminal do XLXD. Cada mudança cria backup e auditoria; não existe terminal Linux nesta página.</p></div><span class="badge">Backup automático · confirmação por senha</span></div><?php if(!$accessOk):?><div class="msg bad">Não foi possível ler as configurações de acesso.</div><?php else:?><div class="maintenance"><div class="mini"><h3>Lista branca</h3><p class="muted">Regras ativas: <strong><?=h(implode(', ',(array)($access['whitelist']??[]))?:'nenhuma')?></strong></p><?php if(in_array('*',(array)($access['whitelist']??[]),true)):?><p class="warn">Acesso aberto: a regra <code>*</code> autoriza todos.</p><?php endif;?><form method="post"><input type="hidden" name="csrf" value="<?=h($token)?>"><input type="hidden" name="action" value="access-add-white"><input name="access_rule" maxlength="9" placeholder="PU2PNY ou PU2*" required><input class="password" type="password" name="access_password" placeholder="Confirme sua senha" required><label class="confirm"><input type="checkbox" name="confirm_access" value="yes" required> Confirmo a alteração.</label><button class="good">Adicionar regra</button></form><details class="new"><summary>Remover regra</summary><form method="post"><input type="hidden" name="csrf" value="<?=h($token)?>"><input type="hidden" name="action" value="access-delete-white"><input name="access_rule" maxlength="9" required placeholder="Regra exata"><input class="password" type="password" name="access_password" placeholder="Confirme sua senha" required><label class="confirm"><input type="checkbox" name="confirm_access" value="yes" required> Confirmo a remoção.</label><button class="danger">Remover</button></form></details></div><div class="mini"><h3>Lista negra</h3><p class="muted">Bloqueios ativos: <strong><?=h(implode(', ',(array)($access['blacklist']??[]))?:'nenhum')?></strong></p><form method="post"><input type="hidden" name="csrf" value="<?=h($token)?>"><input type="hidden" name="action" value="access-add-black"><input name="access_rule" maxlength="9" placeholder="PU5BIF ou PU5*" required><input class="password" type="password" name="access_password" placeholder="Confirme sua senha" required><label class="confirm"><input type="checkbox" name="confirm_access" value="yes" required> Confirmo o bloqueio.</label><button class="danger">Bloquear</button></form><details class="new"><summary>Remover bloqueio</summary><form method="post"><input type="hidden" name="csrf" value="<?=h($token)?>"><input type="hidden" name="action" value="access-delete-black"><input name="access_rule" maxlength="9" required placeholder="Regra exata"><input class="password" type="password" name="access_password" placeholder="Confirme sua senha" required><label class="confirm"><input type="checkbox" name="confirm_access" value="yes" required> Confirmo a remoção.</label><button class="danger">Remover</button></form></details></div><div class="mini"><h3>Terminal XLXD</h3><p class="muted">Endereço: <strong><?=h((string)($access['terminal']['address']??''))?:'padrão'?></strong> · módulos: <strong><?=h((string)($access['terminal']['modules']??'*'))?></strong></p><form method="post"><input type="hidden" name="csrf" value="<?=h($token)?>"><input type="hidden" name="action" value="access-save-terminal"><label>IP, host ou endereço anunciado<input name="terminal_address" maxlength="253" value="<?=h((string)($access['terminal']['address']??''))?>" placeholder="vazio = padrão"></label><label>Módulos aceitos<input name="terminal_modules" maxlength="27" value="<?=h((string)($access['terminal']['modules']??'*'))?>" placeholder="ABCDE ou *" required></label><input class="password" type="password" name="access_password" placeholder="Confirme sua senha" required><label class="confirm"><input type="checkbox" name="confirm_access" value="yes" required> Confirmo a alteração.</label><button>Salvar terminal</button></form></div></div><p class="sub">As regras são gravadas com backup. Use o reinício protegido do XLXD somente depois de conferir a configuração.</p><?php endif;?></section>
'''
