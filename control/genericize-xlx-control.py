...
access_post = r''' if(in_array($a,['access-add-white','access-delete-white','access-add-black','access-delete-black'],true)){
 ...
 if($a==='access-save-terminal'){
  ...
  else{[$aok,$aj,$detail]=jr('access-save-terminal',[$address,$modules]);if($aok){$access=$aj;$accessOk=true;$msg='XLX Interlink salvo. Reinicie o XLXD somente se quiser aplicar imediatamente.';audit('terminal_save');}else{$msg='Não foi possível salvar: '.mb_substr($detail,0,140);$bad=true;audit('terminal_fail');}}
 }
'''
